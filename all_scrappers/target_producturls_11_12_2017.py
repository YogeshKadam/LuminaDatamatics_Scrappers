...
class EbaySpider(scrapy.Spider):
    imgcount = 1
    name = "target_producturls_11_12_2017"
    allowed_domains = []
        ...
        for dict in titleurlslist:
            for key,values in dict.iteritems():
                try:
                    ...
                    if key == 'url':
                        item=EbayItems()
                        titleurl="https://www.target.com" + values
                        item['titleurl']=titleurl
                        yield item
                        #print titleurl
                except:
                    #print key.encode('utf-8'), values.encode('utf-8')
                    if key.encode('utf-8') == 'url':
                        item=EbayItems()
                        titleurl="https://www.target.com" + values.encode('utf-8')
                        item['titleurl']=titleurl
                        yield item
                        #print titleurl
                
        #titleurlslist=response.xpath('//div[@class="products products-tile"]/ul/li')
        #if len(titleurlslist)>0:
        #    for titleurls in titleurlslist:
        #        titleurl=titleurls.xpath('div[1]/div[2]/div[2]/div/a/@href').extract()
