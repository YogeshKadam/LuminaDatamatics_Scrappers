...
class ShoesSpider(scrapy.Spider):
    imgcount = 1
    name = "shoes_producturls_12_12_2017"
    allowed_domains = []
    ...
    def parse(self,response):
        ...
        urls=[
"https://www.shoes.com/womens-boots-all/category_252",
]
        ...
        for url in urls:
            try:
                ...
                i=2
                while i<=144:
                    urlpart1=url+ "?page="+str(i)
                    #urlpart2="&category=5xu2g&visitorId=016044118A54020190B1854E0B38F460&channel=web&pageId=/c/"
                    req = scrapy.Request( urlpart1 , callback=self.parse1, meta={'main_url':urlpart1})
                    i += 1
    ...
    def parse1(self,response):
        titleurlslist=response.xpath('//article[@class="main_content"]/div[@class="pt_grid"]/div')
        if len(titleurlslist)>0:
           for titleurls in titleurlslist:
               titleurl=titleurls.xpath('div[4]/a/@href').extract()[0]
               #position=titleurls.xpath('div[1]/span/text()').extract()
               item=ShoesItems()
               item['titleurl']="https://www.shoes.com"+titleurl
