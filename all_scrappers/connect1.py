...
file2=open("preprocessedcomments.txt","w")
...
def main():
	...
	while 1:
		...
		if not line:
			...
			gingerpass(line)
			#print (line)
	"""while original_text is not None :
	#while '\n' in original_text :
		if original_text.strip('flag') :
			gingerpass(original_text)
			#print (original_text)
			quit()
		for line in original_text:  # This keeps reading the file
        		if line.strip() == 'flag':
				gingerpass(original_text)
				#quit()
			else :
			gingerpass(original_text)
			#print (original_text)
			#file2.write(original_text + " ")
		quit()"""
	"""for line in original_text:
		if line.strip('flag'):  # Or whatever test is needed
			print ("Hello")
			gingerpass(original_text)
			break
