...
class Spider(scrapy.Spider):
    ...
    name = "kohls_producturls_category2"
    allowed_domain=[]
    ...
    def parse(self, response):
        ...
        while i <=1080:
            ...
            req = scrapy.Request( urls+urlpart , callback=self.parse1, headers={'origin': 'https://www.target.com' , 'accept-encoding': 'gzip, deflate, br' , 'accept-language': 'en-US,en;q=0.9' , 'user-agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 Safari/537.36' , 'accept': 'application/json, text/javascript, */*; q=0.01' , 'referer': 'https://www.target.com/' })
            yield req
            time.sleep(.1)
            i += 60

    def parse1(self, response):
        #print response.xpath('/html/body/div[@id="container"]/div[2]/div[2]/div[2]/div').extract()
        titleurllist=response.xpath('/html/body/div[@id="container"]/div[2]/div[2]/div[2]/noscript/div[@class="products_matrix clearfix"]/ul/li')
        for urls in titleurllist:
            if urls.xpath('div/div[@class="prod_nameBlock"]/p/a/@href').extract():
                item=KohlsItems()
                item['titleurl']= "https://www.kohls.com" + urls.xpath('div/div[@class="prod_nameBlock"]/p/a/@href').extract()[0]
                yield item